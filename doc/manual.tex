...
\newcommand{\currPhycasMajorVersion}{2.0}			% TODO: CHECK PRIOR TO NEW RELEASE
\newcommand{\currMajorVersionReleaseDate}{4 July 2014} % TODO: CHECK PRIOR TO NEW RELEASE

\newcommand{\currPhycasMinorVersion}{2.0.1}			% TODO: CHECK PRIOR TO NEW RELEASE
\newcommand{\currMinorVersionReleaseDate}{5 August 2014} % TODO: CHECK PRIOR TO NEW RELEASE
\newcommand{\currPyVersion}{2.7}
\newcommand{\windistrfilename}{phycas-2.0.1-win.zip}			% TODO: CHECK PRIOR TO NEW RELEASE
\newcommand{\macdistrfilename}{phycas-2.0.1-mac.tar.gz}			% TODO: CHECK PRIOR TO NEW RELEASE
\newcommand{\linuxdistrfilename}{phycas-2.0.1-src.tar.gz}			% TODO: CHECK PRIOR TO NEW RELEASE
...
Note that updater diagnostics reports are generated at 100, 300, 700, 1500, 3100, etc., cycles. The value 100 comes from \opt{mcmc}{adapt\_first}, and the waiting time for each subsequent report is twice the length of waiting time since the last report. The reasoning behind this schedule is that updater diagnostics reports are produced when all slice samplers in use are tuned\index{slice sampler!tuning}, where tuning a slice sampler involves adjusting its horizontal unit slice width to be more efficient. As the analysis proceeds, less tuning is necessary, so the interval between adaptations increases over time.
